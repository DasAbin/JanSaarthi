...
export class FormService {
  ...
  async explainForm(req: FormExplainRequest): Promise<FormExplainResponse> {
...
Be practical and helpful. Use simple language. Give real examples. Identify at least 5-10 fields if present.`;
    ...
    try {
      const jsonStr = await askLLM(prompt, { json: true });
      const result = JSON.parse(jsonStr);

      return {
        title: result.title || "Government Form",
        description: result.description || "Government application form",
        fields: Array.isArray(result.fields) ? result.fields.map((f: Partial<FormField>) => ({
  ...
  private extractBasicFields(text: string): FormField[] {
