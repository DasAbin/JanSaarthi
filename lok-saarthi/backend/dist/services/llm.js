...
async function getWorkingGeminiModelId() {
    if (cachedGeminiModelId)
    ...
    try {
        ...
        const withGenerate = models.filter((m) => m.name && (m.supportedGenerationMethods || []).includes("generateContent"));
        // Prefer flash > pro, strip "models/" prefix
        const byPreference = withGenerate.sort((a, b) => {
            const an = (a.name || "").toLowerCase();
            const bn = (b.name || "").toLowerCase();
            if (an.includes("flash") && !bn.includes("flash"))
                return -1;
            if (!an.includes("flash") && bn.includes("flash"))
                return 1;
            return 0;
        ...
        console.warn("[LLM] ListModels error", e?.message);
    }
...
async function askLLM(prompt, options) {
    ...
    if (opts.preferFast && !opts.imageBase64) {
        ...
        if (openai) {
            try {
                ...
                if (wantsJson && output)
                    return stripJsonMarkdown(output);
                if (output)
                    return output;
            }
            catch {
    ...
    try {
        ...
        console.error("[LLM] Gemini failed", err?.message || err);
    }
    ...
    if (openai) {
        try {
            ...
            if (wantsJson && output)
                output = stripJsonMarkdown(output);
            return output;
        }
        catch (openaiErr) {
            ...
            console.error("[LLM] OpenAI fallback failed", openaiErr?.message || openaiErr);
        }
    }
    else {
